	...
	public class World : IWorld
	{
		...
		public IMovement Simulate(IBox box, float x, float y, Func<ICollision, ICollisionResponse> filter)
		{
			...
			var result = new Movement()
			{
				Origin = origin,
				Goal = destination,
				Destination = this.Simulate(hits, new List<IBox>() { box }, (Box)box, origin, destination, filter),
				Hits = hits,
			};
		...
		private RectangleF Simulate(List<IHit> hits, List<IBox> ignoring, Box box, RectangleF origin,
			RectangleF destination, Func<ICollision, ICollisionResponse> filter)
		{
