...
namespace Dolanan.AsepritePipeline
{
	...
	public class AsepriteWriter : ContentTypeWriter<TWrite>
	{
		...
		{
			FrameElement[] frameValues = value.Frames.ToArray();
			
			// Animation Count
			output.Write(value.Meta.FrameTags.Length);
			
			// Animation
			foreach (FrameTag tag in value.Meta.FrameTags)
			{
				// Animation Name
				...
				{
					for (int i = (int)tag.From; i <= (int)tag.To; i++)
					{
						// Frame index
						output.Write(i);
						// duration
				...
				{
					for (int i = (int)tag.To; i <= (int)tag.From; i--)
					{
						// Frame index
						output.Write(i);
						// duration
			...
			foreach (Slice slice in value.Meta.Slices)
			{
				// slice name
				...
				int bit = slice.Keys[0].Bounds != null ? 1 : 0;
